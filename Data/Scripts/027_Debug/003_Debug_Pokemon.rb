module PokemonDebugMixin
  def pbPokemonDebugCommands(settingUpBattle=false)
    ...
    commands.add("main","sethue",_INTL("Set hue"))
    ...
    commands.add("main","cosmetic",_INTL("Cosmetic info..."))
      ...
      commands.add("cosmetic","setdelta",_INTL("Set Delta"))
  ...
  def pbPokemonDebugActions(command,pkmn,pkmnid,heldpoke=nil,settingUpBattle=false)
    ...
    when "sethue"
        ...
        newhue = pbMessageChooseNumber(
           _INTL("Set {1}'s Hue.",pkmn.name),params) { pbUpdate }
        if newhue!=pkmn.hue
          pkmn.hue = newhue
          pbDisplay(_INTL("{1} is now the color.",pkmn.hue))
          pbRefreshSingle(pkmnid)
      end
    #===========================================================================
    when "setstatus"
   ...
   #===========================================================================
    when "setdelta"
      cmd = 0
      loop do
        olddelta = (pkmn.isDeltap?) ? _INTL("delta") : _INTL("normal")
        msg = [_INTL("Delta ({1}) is natural.",olddelta),
               _INTL("Delta ({1}) is being forced.",olddelta)][pkmn.deltap!=nil ? 1 : 0]
        cmd = pbShowCommands(msg,[
             _INTL("Make Delta"),
             _INTL("Make normal"),
             _INTL("Remove override")],cmd)
        ...
        when 0   # Make Delta
          pkmn.makeDeltap
          pkmn.pbRandomMoves
          pkmn.abilityOverride = rand(PBAbilities.maxValue)
          pkmn.typeOverride1 = rand(PBTypes.maxValue)
          pkmn.typeOverride2 = rand(PBTypes.maxValue)
        when 1   # Make normal
          pkmn.makeNotDeltap
        when 2   # Remove override
          pkmn.deltaflag = nil
