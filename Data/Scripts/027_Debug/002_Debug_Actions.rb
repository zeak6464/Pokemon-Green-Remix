...
#===============================================================================
# Give the player a party of ALL Pokémon.
# For demonstration purposes only, not to be used in a real game.
#===============================================================================
def pbCreateAllPokemon
  party = []
  added = 0
  for id in 1..151
    cname = getConstantName(PBSpecies,id)
    species = id
    if added == PBSpecies.maxValue
      pbMessage(_INTL("Done."))
      break
     else 
      $Trainer.party[added] = pbNewPkmn(species,100)
      added += 1
      pbMessage(_INTL("{1} has been Added.\\^",species))
    end
  end
end
