...
#===============================================================================
#
# Instructions: Put this script bellow BW_PokedexMenu. Download the file BW Pokédex.rar and
...
# Changes the sprite position when using the Search Mode of the Dex
    @sprites["icon"].x = 111
    @sprites["icon"].y = 211
...
# Changes the position of the Dex Name's, as well as the color of the text, to 
# mimic the one used in BW
    textpos = [
       [dexname,Graphics.width/5,8,2,Color.new(222,222,222),Color.new(132,132,132)]
    ]
# Changes the position of the Species Name's, as well as the color of the text, 
# to mimic the one used in BW    
    textpos.push([PBSpecies.getName(iconspecies),109,327,2,base,shadow]) if iconspecies>0
    if @searchResults
...
# mode, as well as the color of the text, to mimic the one used in BW    
      textpos.push([_INTL("Search results"),127,55,2,base,shadow])
...
# text, to mimic the one used in BW
      textpos.push([_INTL("SEEN"),127,55,0,base,shadow])
      textpos.push([$Trainer.pokedexSeen(pbGetPokedexRegion).to_s,242,55,1,base,shadow])
      textpos.push([_INTL("OWNED"),335,55,0,base,shadow])
      textpos.push([$Trainer.pokedexOwned(pbGetPokedexRegion).to_s,495,55,1,base,shadow])
    end
    # Draw all text
    ...
    if itemlist.top_item+itemlist.page_item_max<itemlist.itemCount
      overlay.blt(468,307,@sliderbitmap.bitmap,Rect.new(0,30,40,30))
      showslider = true
    end
    # Draw slider box
    if showslider
...
# Changes the color of the text, to the one used in BW
    ...
    textpos = [
       [_INTL("Search Mode"),Graphics.width/5,8,2,base,shadow],
