...
class Game_Character
  def bush_depth
    ...
    if $game_map.valid?(xbehind,ybehind)
      ...
      if @jump_count <= 0 and self.map.bush?(@x, @y) and !moving?
        return 12 
      elsif @jump_count <= 0 and self.map.bush?(@x, @y)
        return 12
      #added for water
      elsif @jump_count <= 0 and self.map.water?(@x, @y) and !moving?
        return 12 
      elsif @jump_count <= 0 and self.map.water?(@x, @y)
        return 12
      else
        return 0
      end
...
#===============================================================================
# adds new method water?(x,y) to the class Game_Map (originally defined in script Game_Map)
#===============================================================================
class Game_Map
  def water?(x,y)
    if @map_id != 0
